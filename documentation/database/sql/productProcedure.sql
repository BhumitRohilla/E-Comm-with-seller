...
--create table Product_Price (ProductId int primary key, price decimal(12,2) not null, foreign key(ProductId) references Product(ProductId));
...
alter procedure insertIntoProduct @sellerName varchar(50),@title varchar(100),@date date,@stock int,@userReview int,@status varchar(10) ,@img varchar(50),@active bit,@tag varchar(2000),@about varchar(8000),@price decimal(12,2)
as
begin
	declare @id int;
	...
	insert into Product_Price values(@id,@price);
end
...
returns table
as
	return select Product.*,tag,[about-game],price from Product inner join (select c1.*,price from (select Product_Tag.*,[about-game] from Product_Tag inner join About_Product on Product_Tag.productId = About_Product.ProductId) as c1 inner join Product_Price on Product_Price.ProductId = c1.productId) as c2 on  c2.ProductId = Product.ProductId where active = 1 order by ProductId OFFSET @start ROWS FETCH NEXT @length ROWS ONLY;
...
begin
	...
	begin try
		...
		if @stock < 0 
			throw 50500,'STOCK WENT INTO NEGATIVE',1;
		commit;
	end try
	begin catch
...
returns table
as
	return select Product.*,tag,[about-game],price from Product inner join (select c1.*,price from (select Product_Tag.*,[about-game] from Product_Tag inner join About_Product on Product_Tag.productId = About_Product.ProductId) as c1 inner join Product_Price on Product_Price.ProductId = c1.productId) as c2 on  c2.ProductId = Product.ProductId where active = 1;
...
begin
	begin transaction 
	begin try
		delete cart_item where ProductId = @pid;
		update Product set active = 0 where ProductId = @pid;
		commit;
	end try
	begin catch
		rollback transaction;
