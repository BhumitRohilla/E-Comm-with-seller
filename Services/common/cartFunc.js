...
function addToCartAndRemoveStock(userName,pid){
    let result;
    if(process.env.USESQL == 'true'){
        result =  sqlCart.addToCartAndRemoveStock(userName,pid);
    }else{
        result =  mongoCart.addToCartAndRemoveStock(userName,pid);
    }
    return result;
}

module.exports = {getQuantity,addToCart,getUserCart,getUserCartItem,removeFromCart,deleteFromCart,getTotalPriceOfCartUserName,addToCartAndRemoveStock};